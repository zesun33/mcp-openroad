#!/usr/bin/env bash
# verify.sh — Verification flow for @zesun33/mcp-openroad.
#
...
for arg in "$@"; do
  case "$arg" in
    ...
    --gate=*) GATE="${arg#--gate=}" ;;
    -h|--help)
      echo "verify.sh — mcp-openroad verification suite"
      ...
      echo "                   4  fixture integration (podman openroad pnr & timing)"
...
run_gate_4() {
  echo "--- Gate 4: Fixture Integration (Podman OpenROAD P&R & Timing) ---"
  npx tsx --test tests/integration.test.ts || fail 4 "Integration tests failed"
  rm -f counter_fp.def counter_routed.def
  pass 4 "Podman container integration passed on P&R fixtures"
}

run_gate_5() {
  echo "--- Gate 5: Protocol Contract (stdio JSON-RPC) ---"
  RESPONSE=$(echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | node dist/index.js)
  echo "${RESPONSE}" | grep -q "openroad_pnr" || fail 5 "Tools list does not contain openroad_pnr"
  echo "${RESPONSE}" | grep -q "openroad_floorplan" || fail 5 "Tools list does not contain openroad_floorplan"
...
run_gate_6() {
  ...
  grep -q "openroad_pnr" README.md || fail 6 "README missing openroad_pnr docs"
...
case "${GATE}" in
  ...
  "")
    ...
    run_gate_6
    echo ""
    echo -e "\033[0;32m=== All Gates Cleared: mcp-openroad Verified ===\033[0m"
    ;;
  *)
